...
选择本地测试服或线上服务器后，打开“关卡开关”。填写地图ID加入关闭列表，点击保存；移除列表项并保存可恢复该地图原有的开放规则。配置分别保存在所选环境，不会自动同步至另一个环境。

新版服务器在新建房间、修改地图、开始战斗时检查配置，随机选图也会排除关闭项；已进行的战斗不会被强制中断。此功能只限制服务器已有地图池，不能开启未支持的玩法，也不代替玩家关卡解锁进度。
