...
& 'runtime-local/go-online/latency-probe.exe' -endpoint 'tls://114.55.133.88:19091' -count 3 -cert 'runtime-local/go-online/origin.crt' -output $reportPath
if ($LASTEXITCODE -ne 0) { throw 'Direct TLS probe failed; launcher settings were not changed.' }
$report = Get-Content $reportPath -Raw | ConvertFrom-Json
$result = $report.summary.'tls://114.55.133.88:19091'
...
foreach ($path in $paths) {
    ...
    $settings.url = 'tls://114.55.133.88:19091'
