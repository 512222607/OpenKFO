...
foreach ($file in @('launcher/功夫小子登录器.exe','GM管理器/GM管理器.exe','GM管理器/kungfu-desktop-admin.exe','GM管理器/flutter_windows.dll','launcher-components/OnlineBridge.exe','launcher-components/LoginSkin.dll','launcher-components/LoginSkinHost.exe')) {
...
foreach ($name in @('功夫小子登录器.exe','item-manager','GM管理器','launcher-components','登录器组件','打开道具管理器.cmd','打开GM管理器.cmd')) {
...
Copy-Item -LiteralPath "$dist/GM管理器" -Destination $gameRoot -Recurse
...
@{ root = $adminRoot } | ConvertTo-Json | Set-Content -LiteralPath "$gameRoot/GM管理器/gm-settings.json" -Encoding utf8
