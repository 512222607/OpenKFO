...
internal sealed class UpdateForm : Form
{
    ...
    readonly bool autoCheck;
    readonly Button local = new() { Text = "选择本地更新清单…", AutoSize = true };
    readonly Button skipUpdate = new() { Name = "skipUpdate", Text = "跳过本次更新", AutoSize = true, Visible = false };
    internal UpdateForm(string kind, string target, Uri uri, string? bridge, string? launcher = null, string? restartRoot = null, string? offlineManifest = null, bool autoCheck = false)
    {
        ...
        this.autoCheck = autoCheck && kind == "gm";
        ...
        actions.Controls.Add(local); actions.Controls.Add(skipUpdate); layout.Controls.Add(actions, 0, 3);
        skipUpdate.Click += (_, _) => {
            if (kind != "launcher" || working || manifest == null || File.Exists(Path.Combine(UpdateEngine.StateDirectory(target, kind), "journal.json"))) return;
            Skipped = true; Close();
        };
        if (this.autoCheck) {
            var skip = new Button { Text = "跳过更新，继续使用", AutoSize = true };
            skip.Click += (_, _) => { if (!working) Close(); };
    ...
    async Task Check()
    {
        local.Enabled = false; install.Enabled = false; skipUpdate.Visible = false; manifest = null;
        try
        {
            ...
            if (current) { Completed = true; status.Text = "已经是最新版本"; if (kind != "gm" || autoCheck) { Close(); return; } notes.Text = manifest.Notes; return; }
            install.Text = offlineManifest == null ? "下载并更新" : "安装本地更新包";
            skipUpdate.Visible = kind == "launcher"; skipUpdate.Enabled = true;
        ...
        catch (Exception e) { status.Text = "检查失败"; notes.Text = PublicError(e); DiagnosticDialog.Show(this, status.Text, notes.Text + "\r\n\r\n" + DiagnosticDialog.Details(e)); }
        finally { local.Enabled = true; }
    }
    async Task Install()
    {
        working = true; install.Enabled = false; local.Enabled = false; skipUpdate.Enabled = false; try
        {
        ...
        catch (Exception e) { status.Text = "更新失败，未完成替换"; notes.Text = PublicError(e); DiagnosticDialog.Show(this, status.Text, notes.Text + "\r\n\r\n" + DiagnosticDialog.Details(e)); install.Enabled = true; working = false; }
        finally { local.Enabled = true; skipUpdate.Enabled = !working && !File.Exists(Path.Combine(UpdateEngine.StateDirectory(target, kind), "journal.json")); }
    }
