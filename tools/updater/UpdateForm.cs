...
internal sealed class UpdateForm : Form
{
    ...
    internal UpdateForm(string kind, string target, Uri uri, string? bridge, string? launcher = null, string? restartRoot = null)
    {
    ...
    async Task Check()
    {
        try
        {
            ...
            byte[] raw;
            try { raw = await UpdateEngine.Download(http, uri, 65536, _ => { }); }
    ...
    async Task Install()
    {
        working = true; install.Enabled = false; try
        {
            ...
            status.Text = "正在安装…"; await Task.Run(() => UpdateEngine.Apply(manifest, files, target, bridge, launcher: launcher));
    ...
    static string PublicError(Exception e) => e is HttpRequestException or TaskCanceledException ? "无法连接更新服务，请检查网络后重试。" : e.Message;
