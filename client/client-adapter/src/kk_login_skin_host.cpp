...
int wmain(int count, wchar_t** args) {
    ...
    HHOOK hook = nullptr;
    for (int attempt = 0; attempt < 120 && WaitForSingleObject(process, 0) == WAIT_TIMEOUT; ++attempt) {
        EnumWindows(findGame, processId);
        if (loginWindow) {
            // The existing login dialog has its own UI thread, distinct
            // from GAMECLIENT. Subclass only on the dialog's owning thread.
            hook = SetWindowsHookExW(WH_CALLWNDPROC, hookFunction, module, GetWindowThreadProcessId(loginWindow, nullptr));
            if (hook) break;
        }
        Sleep(500);
    }
    if (!hook) { FreeLibrary(module); CloseHandle(lock); CloseHandle(process); return 5; }
    while (WaitForSingleObject(process, 2000) == WAIT_TIMEOUT) {
        DWORD_PTR ignored;
        if (!IsWindow(loginWindow)) break;
        SendMessageTimeoutW(loginWindow, WM_APP + 0x3b7, 0x4b4b534b, 0, SMTO_ABORTIFHUNG | SMTO_BLOCK, 1000, &ignored);
    }
    UnhookWindowsHookEx(hook); FreeLibrary(module); CloseHandle(lock); CloseHandle(process);
    return 0;
}
