...
func TestMailboxLocalDatabase(t *testing.T) {
	...
	if p, err := s.Mailbox(uid + 1); err != nil || len(p) != 0 {
		t.Fatal("foreign inbox", err)
	}
	if _, err := s.ReadMail(uid+1, id); err != sql.ErrNoRows {
		t.Fatal("foreign read", err)
	}
	if err := s.DeleteMail(uid+1, id); err != ErrDenied {
		t.Fatal("foreign delete", err)
	}
	for i := 0; i < 2; i++ {
		p, err = s.ReadMail(uid, id)
	...
	for i := 0; i < 2; i++ {
		if err = s.DeleteMail(uid, id); err != nil {
			t.Fatal(err)
		}
	}
	if p, err = s.Mailbox(uid); err != nil || len(p) != 0 {
		t.Fatal("deleted visible", err)
	}
	if _, err = s.ReadMail(uid, id); err != sql.ErrNoRows {
	...
	if _, e := s.ClaimMailItem(uid+1, key); e == nil {
	...
	for i := 0; i < 2; i++ {
		wg.Add(1)
		go func() { defer wg.Done(); p, e := s.ClaimMailItem(uid, key); results <- p; failures <- e }()
	}
	wg.Wait()
	close(results)
	close(failures)
	...
	if err = s.DeleteMail(uid, mail); err != nil {
		t.Fatal(err)
	}
	if _, err = s.ClaimMailItem(uid, key); err != nil {
	...
	if p, err = s.ClaimMailItem(uid, key); err != nil || len(p) != 0 {
	...
	if _, err = s.ClaimMailItem(uid, key); err != ErrDenied {
