...
func TestStageAccessLocalDatabase(t *testing.T) {
	...
	loaded.FosterPlans = []FosterConfig{{MapID: 8110, ScriptHash: strings.Repeat("b", 64), RuntimeHash: strings.Repeat("c", 64), ConfigHash: strings.Repeat("d", 64), Templates: []string{" Monster", "Monster"}, Plan: protocol.FosterPlan{InitialHP: []float32{8, 75}, PlayerLimit: 6, GlobalLimit: 32, Groups: []protocol.FosterGroup{{SubLimit: 2, GroupLimit: 20, Spawns: []protocol.FosterSpawn{{Template: 0, Direction: 2}}}}}}}
	loaded, err = s.SaveStageAccess(loaded)
	if err != nil {
	...
	loaded, err = s.StageAccess()
	if err != nil || len(loaded.FosterPlans) != 1 || loaded.FosterPlans[0].Plan.InitialHP[1] != 75 || loaded.FosterPlans[0].Templates[0] != " Monster" || loaded.FosterPlans[0].Plan.Groups[0].Spawns[0].Direction != 2 {
