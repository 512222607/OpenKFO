...
func TestPersistedFosterPlan(t *testing.T) {
	...
	a := persistence.StageAccess{ClientHash: hash, PVEMaps: []uint32{8110}, Requirements: []persistence.StageTitleRequirement{{MapID: 8110, Name: "Stage"}}, FosterPlans: []persistence.FosterConfig{{MapID: 8110, ScriptHash: hash, RuntimeHash: hash, ConfigHash: hash, Templates: []string{"Monster"}, Plan: protocol.FosterPlan{PlayerLimit: 6, GlobalLimit: 32, Groups: []protocol.FosterGroup{{SubLimit: 2, GroupLimit: 20, Spawns: []protocol.FosterSpawn{{Template: 0, Direction: 2}}}}}}}}
	...
	if err != nil || next.Groups[0].Spawns[0].Direction != 4 || next.Groups[0].SubLimit != 1 {
		t.Fatal("next battle missed GM update", err)
	}
	for _, scenario := range []string{"version", "disabled", "missing", "empty-party", "large-party", "unknown-map"} {
