...
from .spf2 import parse_paths, decode_config
...
class ClientConfig:
    ...
    def __init__(self, package):
        ...
        if not self.data.startswith(b'SGDP Package File.\0') or struct.unpack_from('<I',self.data,36)[0]!=2:
            raise ValueError('unsupported_config_package')
        ...
        paths=parse_paths(self.data[tree:table],count)
        ...
        for index,raw in paths.items():
            key=raw.replace('\\','/').lstrip('/').casefold()
    ...
    def xml(self, name):
        ...
        raw=decode_config(self.data[offset+4:offset+size+4])
