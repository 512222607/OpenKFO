    ...
    def handle(self, c, message):
        ...
        if ident in (1010, 2010):
            ...
            if ident == 1010:
                ...
                result = packets.account_packets(self.store.snapshot(uid)[3], self.game_port,gold=self.store.gold_balance(uid))
                # Native 1230 -> A26F80 -> core+48 (txtTicket); 1250 is txtCoupon.
                if self.store.ticket_balance(uid):
                    # Deliver before the 7080/7070 server handoff, while LS is open.
                    result.insert(2, Message(1230, struct.pack('<I', self.store.ticket_balance(uid))))
                return result
        ...
        if ident==1232 and c.phase in (Phase.LOBBY,Phase.ROOM):
            self.require(not p,'ticket balance query length')
            # GS 1230 -> 8256B0 refreshes txtTicket after writing core+48.
            return [Message(1230,struct.pack('<I',self.store.ticket_balance(c.uid)))]
        ...
        if ident in (9040,9041) and c.phase in (Phase.LOBBY,Phase.ROOM):
            ...
            if ident==9040:
                ...
                try:result=self.store.purchase_gold_once(c.uid,operation,p)
                    ...
                    self.last_purchase_event=dict(event='purchase_rejected',reason=str(exc),**decode_known(ident,p))
