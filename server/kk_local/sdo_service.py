...
from .maps import MapCatalog, contained
...
async def run(args):
    ...
    game_image=contained(root,getattr(args,'client_exe','gfxz-lab.exe'))
    if not game_image.is_file():raise ValueError('client executable missing: '+str(game_image))
    store=Store(args.database);game_identity=WindowsNativeVerifier(game_image)
...
def main():
    ...
    p.add_argument('--client-exe',default='gfxz-lab.exe',help='Exact client executable relative to client-root; process identity checks remain enabled')
