...
class Store:
    def __init__(self, path: str):
        ...
        self.db.executescript('''
          ...
          CREATE TABLE IF NOT EXISTS admin_wallet_operations(
            operation TEXT PRIMARY KEY, request TEXT NOT NULL, result TEXT NOT NULL);
    ...
    def ticket_balance(self, uid):
        row = self.db.execute('SELECT balance FROM ticket_wallet WHERE uid=?', (uid,)).fetchone()
        return row[0] if row else 0
    ...
    def _offer_record(self, key):
        ...
        if (not r.raw[48] or r.raw[46] or r.raw[49] or r.requirement not in ((0,0),(1,0)) or r.u32(88) or r.u32(77)
                or (bool(r.u32(30)) == bool(r.u32(38)))
                or not 0 < (r.u32(30) or r.u32(38)) <= 2147483647
                or r.u32(34) != r.u32(30) or r.u32(42) != r.u32(38)):
            raise ValueError('offer requires unsupported settlement rules')
        return r

    def enable_gold_offer(self, key, grant_template):
        if not self._offer_record(key).u32(30):
            raise ValueError("gold offer required")
        return self.enable_shop_offer(key, grant_template)

    def enable_shop_offer(self, key, grant_template):
    ...
    def purchase_gold_once(self, uid, operation_id, request):
        ...
        try:
            ...
            if old:
                ...
                balance=self.gold_balance(uid) if ShopRecord(old[3]).u32(30) else self.ticket_balance(uid)
            ...
            # Native 85D4D4/85D4FE: 109 spends points, 111 spends gold;
            # 110 is coupon-only spending and is not supported here.
            if (struct.unpack_from('<I',request)[0]!=(111 if r.u32(30) else 109)
                    ...
                    or struct.unpack_from('<I',request,157)[0]!=r.u32(38)
                    or any(struct.unpack_from('<I',request,off)[0] for off in (153,161,165))):
                raise ValueError('unsupported or mismatched purchase terms')
            wallet = 'gold_wallet' if r.u32(30) else 'ticket_wallet'
            price = r.u32(30) or r.u32(38)
            balance = self.gold_balance(uid) if r.u32(30) else self.ticket_balance(uid)
            if balance < price:raise ValueError('insufficient balance')
            ...
            balance -= price
            self.db.execute(f'INSERT OR REPLACE INTO {wallet} VALUES(?,?)',(uid,balance))
    ...
    def provision_local(self, uid, account, nickname=None):
        ...
        with transaction:
            ...
            for i, (prop, kind, slot) in enumerate(items):
                ...
                # Native unstarted equipment duration is hours, not item count.
                struct.pack_into('<I', row, 13, 365 * 24)
