...
class AuthServer:
    ...
    async def dispatch(self, request):
        ...
        fields={'register':({'account','password'},{'nickname'}),
                ...
                'login_legacy_sha256':({'account','password'},set()),
        ...
        if not required<=set(args) or set(args)-required-optional:
            raise AuthError('invalid_request')
        if op in ('register','login','login_legacy_sha256'):
            now=time.monotonic()
        ...
        if op=='login_legacy_sha256': return await self.manager.login(**args,legacy_sha256=True)
    ...
    async def _accept(self, reader, writer):
        ...
        try:
            ...
            if isinstance(request,dict) and request.get('operation') in ('register','login','login_legacy_sha256','regions','select_region','bind_client','status','logout'):
