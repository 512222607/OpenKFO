...
def gold_purchase_result(balance, item_record, catalog_record):
    ...
    return [Message(1240 if struct.unpack_from('<I',catalog_record,30)[0] else 1230,struct.pack('<I',balance))]+([Message(2160,bytes(item_record))] if item_record is not None else [])+[Message(9050,bytes(catalog_record))]
