"""Local-only account registration and password setup.
...
def main():
    ...
    p.add_argument('operation',choices=['set-password','register'])
    p.add_argument('account')
    args=p.parse_args()
    first=getpass.getpass('New local game password (8-128 characters): ')
    ...
    try:
        if args.operation=='register':
            result=asyncio.run(manager.register(args.account,first))
            print('Local account registered. UID:',result['uid'])
        else:
