...
# Fixed wire-format prefix observed in this client's SDError.dll; not a secret.
LEGACY_PASSWORD_PREFIX = b'xfmRn9z7K1wTfvBYhpCwZmE8yLWN1oLv'

def legacy_password_digest(encoded):
    return hashlib.sha256(LEGACY_PASSWORD_PREFIX + encoded).hexdigest()
...
def password_bytes(password):
    if not isinstance(password,str) or not 8<=len(password)<=128:
        raise AuthError('password_length_8_to_128_required')
    try:
...
class AuthManager:
    def __init__(self, store, regions, *, clock=time.time, native_verifier=None):
        ...
        self.db.executescript('''
          ...
          CREATE TABLE IF NOT EXISTS auth_legacy_credentials(
            uid INTEGER PRIMARY KEY REFERENCES auth_credentials(uid) ON DELETE CASCADE,
            normalized_account TEXT NOT NULL UNIQUE,
            algorithm TEXT NOT NULL, salt BLOB NOT NULL CHECK(length(salt)=16),
            digest BLOB NOT NULL CHECK(length(digest)=32));
    ...
    async def register(self, account, password, nickname=None):
        ...
        legacy_digest=await self._hash(legacy_password_digest(encoded).encode('ascii'),legacy_salt)
        ...
        try:
            ...
            self.db.execute('INSERT INTO auth_legacy_credentials VALUES(?,?,?,?,?)',(uid,name,ALGORITHM,legacy_salt,legacy_digest))
            self.db.commit()
    ...
    async def login(self, account, password, *, legacy_sha256=False):
        name=normalize_account(account)
        if legacy_sha256:
            if not isinstance(password,str) or not re.fullmatch(r'[0-9a-fA-F]{64}',password):
                raise AuthError('invalid_credentials')
            password=password.lower()
        encoded=password_bytes(password)
        # Separate verifier domain: a legacy wire digest cannot be used as the
        # plaintext password. Both domains share lockout and session checks.
        table='auth_legacy_credentials' if legacy_sha256 else 'auth_credentials'
        query='SELECT uid,algorithm,salt,digest FROM '+table+' WHERE normalized_account=?'
        now=int(self.clock())
        with self.db:
            self._cleanup(now)
        ...
        try:
            ...
            current=self.db.execute(query,(name,)).fetchone()
    ...
    async def set_local_password(self, account, password):
        ...
        with self.db:
            ...
            self.db.execute('INSERT INTO auth_legacy_credentials VALUES(?,?,?,?,?) ON CONFLICT(uid) DO UPDATE SET normalized_account=excluded.normalized_account,algorithm=excluded.algorithm,salt=excluded.salt,digest=excluded.digest',
                            (uid,name,ALGORITHM,legacy_salt,legacy_digest))
            self.db.execute('DELETE FROM auth_failures WHERE account=?',(name,))
