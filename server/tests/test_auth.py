...
class AuthTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_legacy_sha256_is_separate_salted_and_reset_revokes_both(self):
        await self.registered()
        wire=hashlib.sha256(b'xfmRn9z7K1wTfvBYhpCwZmE8yLWN1oLv'+PASSWORD.encode()).hexdigest()
        request=dict(schema='kk-local-auth-v1',operation='login_legacy_sha256',
                     arguments=dict(account='NewPlayer',password=wire))
        granted=await AuthServer(self.auth).dispatch(request)
        self.assertEqual(granted['uid'],1002)
        with self.assertRaisesRegex(AuthError,'invalid_credentials'):
            await self.auth.login('NewPlayer',hashlib.sha256(PASSWORD.encode()).hexdigest(),legacy_sha256=True)
        with self.assertRaisesRegex(AuthError,'invalid_credentials'):
            await self.auth.login('NewPlayer',wire)
        with self.assertRaisesRegex(AuthError,'invalid_credentials'):
            await self.auth.login('NewPlayer','0'*64,legacy_sha256=True)
        self.assertNotIn(wire.encode(),Path(self.path).read_bytes())
        await self.auth.set_local_password('NewPlayer','Another-Synthetic-Password')
        with self.assertRaises(AuthError): self.auth.list_regions(granted['session'])
        with self.assertRaises(AuthError): await self.auth.login('NewPlayer',wire,legacy_sha256=True)
        with self.assertRaises(AuthError): await self.auth.login('NewPlayer',PASSWORD)
        self.assertEqual((await self.auth.login('NewPlayer','Another-Synthetic-Password'))['uid'],1002)

    async def test_eight_character_password_reset_and_short_rejection(self):
        await self.registered()
        before=self.store.snapshot(1002)
        await self.auth.set_local_password('NewPlayer','Test1234')
        self.assertEqual((await self.auth.login('NewPlayer','Test1234'))['uid'],1002)
        with self.assertRaisesRegex(AuthError,'password_length_8_to_128_required'):
            await self.auth.set_local_password('NewPlayer','short12')
        self.assertEqual(self.store.snapshot(1002),before)
...
class WindowsIdentityTests(unittest.TestCase):
    def test_process_and_kernel_tcp_udp_owner(self):
        # Windows venv's redirector runs the base interpreter image.
        verifier=WindowsNativeVerifier(getattr(sys,'_base_executable',sys.executable))
