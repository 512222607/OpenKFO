...
def setup_offer(s):
    ...
    struct.pack_into('<II',r,30,100,100);r[48]=1;r[83]=1
...
class PurchaseTests(unittest.TestCase):
    ...
    def test_nonzero_title_requirement_is_not_silently_ignored(self):
        changed=bytearray(self.catalog);changed[83]=1;changed[13]=1
        grant=self.s.db.execute('SELECT grant_template FROM gold_offers').fetchone()[0]
        self.s.replace_shop_catalog(25,1,[bytes(changed)])
        with self.assertRaisesRegex(ValueError,'unsupported settlement'):
            self.s.enable_shop_offer(25303301,grant)
        self.assertEqual(self.s.gold_balance(1001),250)
...
class PurchaseNetworkTests(unittest.IsolatedAsyncioTestCase):
    async def test_two_intents_then_insufficient_balance_over_tcp(self):
        await self.check_purchases('gold')

    async def test_ticket_purchase_and_insufficient_balance_over_tcp(self):
        await self.check_purchases('ticket')

    async def check_purchases(self, currency):
        s=Store(':memory:');s.seed_local();req,catalog=setup_offer(s)
        if currency == 'ticket':
            record = bytearray(catalog)
            struct.pack_into('<4I', record, 30, 0, 0, 100, 100)
            grant = s.db.execute('SELECT grant_template FROM gold_offers').fetchone()[0]
            s.replace_shop_catalog(25, 1, [bytes(record)])
            s.enable_shop_offer(25303301, grant)
            with s.db:
                s.db.execute('INSERT INTO ticket_wallet VALUES(1001,250)')
            request = bytearray(req)
            struct.pack_into('<I', request, 0, 109)
            struct.pack_into('<3I', request, 149, 0, 0, 100)
            req, catalog = bytes(request), bytes(record)
        balance_message = 1240 if currency == 'gold' else 1230
        ...
        try:
            ...
            if currency == 'ticket':
                writer.write(encode_game(Message(1232,b'')));await writer.drain()
                self.assertEqual(await receive(),Message(1230,struct.pack('<I',250)))
                wrong_currency = bytearray(req)
                struct.pack_into('<I', wrong_currency, 0, 111)
                writer.write(encode_game(Message(9040,bytes(wrong_currency))));await writer.drain()
            ...
            for balance in (150,50):
                ...
                self.assertEqual([m.id for m in messages],[balance_message,2160,9050])
            ...
            self.assertEqual((s.gold_balance if currency == 'gold' else s.ticket_balance)(1001),50)
            if currency == 'ticket':
                self.assertEqual(s.gold_balance(1001), 250)
