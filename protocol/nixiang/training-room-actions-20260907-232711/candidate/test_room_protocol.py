...
class RoomProtocolTests(unittest.TestCase):
    ...
    def test_incorrect_lengths_and_unknown_handle_are_rejected(self):
        ...
        with self.assertRaises(ValueError):
            self.session.handle(3550, bytes(11), self.control)
        with self.assertRaises(ValueError):
    ...
    def test_real_post_enter_status_does_not_close_session(self):
        opaque = bytes.fromhex('ed0704040004040404040404')
        replies = self.session.handle(3550, opaque, self.control)
        self.assertEqual(replies, [{'message_id': 3550, 'key_index': 0, 'payload_hex': opaque.hex()}])
        self.assertIsNone(self.session.handle(21428, b'', self.control))
