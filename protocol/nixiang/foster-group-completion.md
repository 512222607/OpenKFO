...
服务器的`fosterReceiptsComplete`只核对结束标记、逐组生成数与接收的血量/移除记录。活着移除的实体不补记为死亡；零血量但仍活动的尸体可参与核对；编号复用前的已移除记录按组保留。它不是客户端脚本执行器，也不能单独证明掉落或通关资格。模式10结算另要求已认证房主的完整4110战报、成员槽位/本局身份一致及原因1；原因2仅在无完成标记且全员报告血量为0时按失败配置处理。奖励是模拟器配置规则，不是原版数值或服务端权威伤害模拟。准入仍关闭，生成约束和原生流程验收尚未完成。
