...
def dismiss():
 ...
 time.sleep(.25)
out=R/'evidence'/('uid-lengths-'+time.strftime('%H%M%S')+'.jsonl')
for value in ['0|'+'1'*n for n in [1,8,16,32,36,40,64,128]] + ['1|'+'1'*32,'OK|'+'1'*32]:
 dismiss()
 if g.pid(button)!=p or not g.u.IsWindowVisible(g.W.HWND(button)):break
 (R/'lab-control.json').write_text(json.dumps({'case':'status-candidate-'+value,'body':value}),encoding='utf-8')
