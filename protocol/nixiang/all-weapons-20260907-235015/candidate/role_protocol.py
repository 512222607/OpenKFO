...
class RoleSession:
    ...
    def handle(self, message_id, payload, control):
        ...
        if message_id == 2080:
            ...
            if slot not in (0, 8):
                # The real 2100 handler consumes a uint16 error code. 38 is
                # its generic equipment failure; preserve the login connection.
                return [response(2100, struct.pack('<H', 38), 0)]
            # A second real UI path leaves the destination zero. For an owned
            # type-25 weapon, the client's default equip mapping selects slot 8.
            slot = 8
            inventory = InventoryStore(self.store.path)
