...
def build_start_ack(room_handle=1):
    if not 1 <= room_handle <= 0xffff:
        raise ValueError('Room handle must be a nonzero WORD')
    # raw 0x80E1A0 requires 53 bytes and compares a DWORD with the room's
    # WORD handle. +11 selects the local host seat, not a map identifier.
    # The local one-player lab starts its clock, per-seat ping values and
    # match identity at zero. The final QWORD is the match identity.
    payload = bytearray(START_ACK_SIZE)
    struct.pack_into('<I', payload, 0, room_handle)
    struct.pack_into('<H', payload, 11, 0)
    return bytes(payload)
...
class RoomSession:
    ...
    def handle(self, message_id, payload, control):
        ...
        if message_id == 4030:
            if payload:
                raise ValueError('4030 requires an empty payload')
            if not self.entered or self.request is None or self.started:
                return []
            if self.request[37] != FREE_TRAINING_MODE:
                return []
            self.started = True
            return [response(4080, build_start_ack(self.room_handle))]
        if message_id == 4160:
            if payload:
                raise ValueError('4160 requires an empty payload')
            if not self.started or self.loading_complete:
                return []
            # raw 0x97C9C0 sends 4160 after loading; raw 0x819770 handles
            # 4180 by starting the mode controller and enabling frame updates.
            self.loading_complete = True
            return [response(4180, b'')]
        if message_id == 8040:
            if len(payload) != 14:
                raise ValueError('8040 requires exactly fourteen bytes')
            if not self.loading_complete or self.active:
                return []
            handle, uid, _client_counter = struct.unpack('<HQI', payload)
            if handle != self.room_handle or uid != self.uid:
                raise ValueError('8040 does not identify the local room player')
            # raw 0x81C4E0 enables scene state 4. This QWORD is the match
            # identity from 4080+45, not the request's player UID.
            match_identity = build_start_ack(self.room_handle)[45:53]
            self.active = True
            return [response(8070, struct.pack('<I', self.room_handle) + match_identity)]
        if message_id == 3550:
            if len(payload) != 12:
                raise ValueError('3550 requires exactly twelve bytes')
            # The receiver reads QWORD player UID + DWORD status. Preserve
            # the client's actual status instead of inventing a new field.
            # Real post-3100 traffic includes an opaque/noncanonical UID;
            # this notification is not an authentication request. Rejecting
            # it closes the stream before the following 21428 can be read.
            return [response(3550, payload)]
        if message_id == 3110:
            if payload:
                raise ValueError('3110 requires an empty payload')
            # 3115 dispatches to raw 0x80ED00 / 0x81D920. Both clear the
            # client's room objects and select lobby state 6 without reading
            # a payload. Reset our state as well so lobby polling resumes.
            self.entered = False
            self.started = False
            self.loading_complete = False
            self.active = False
            self.request = None
            return [response(3115, b'')]
        if message_id == 2250:
            if len(payload) != 8:
