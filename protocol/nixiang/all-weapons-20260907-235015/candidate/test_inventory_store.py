...
class InventoryStoreTests(unittest.TestCase):
    ...
    def test_legacy_schema_migration_preserves_rows_and_defaults_to_nonpermanent(self):
        old_path = Path(self.temp.name) / 'legacy.sqlite3'
        with closing(sqlite3.connect(old_path)) as db, db:
            db.execute('CREATE TABLE lab_roles(uid INTEGER PRIMARY KEY,payload BLOB)')
            db.execute('INSERT INTO lab_roles VALUES(?,?)', (1001, self.baseline))
            db.execute('''CREATE TABLE lab_inventory (
                uid INTEGER NOT NULL REFERENCES lab_roles(uid),
                instance_id INTEGER NOT NULL,local_id INTEGER NOT NULL,
                kind INTEGER NOT NULL,payload68 BLOB NOT NULL,
                PRIMARY KEY(uid,instance_id),UNIQUE(uid,local_id))''')
            for item in records(self.baseline[360:]):
                instance_id, kind, local_id = struct.unpack_from('<IBI', item)
                db.execute('INSERT INTO lab_inventory VALUES(?,?,?,?,?)', (1001, instance_id, local_id, kind, item))
            original = db.execute('SELECT * FROM lab_inventory ORDER BY instance_id').fetchall()
        with ThreadPoolExecutor(max_workers=4) as pool:
            list(pool.map(lambda _: InventoryStore(old_path), range(4)))
        with closing(sqlite3.connect(old_path)) as db:
            self.assertEqual(db.execute('SELECT uid,instance_id,local_id,kind,payload68 FROM lab_inventory ORDER BY instance_id').fetchall(), original)
            self.assertEqual(db.execute('SELECT DISTINCT permanent FROM lab_inventory').fetchall(), [(0,)])
            self.assertEqual(db.execute('SELECT payload FROM lab_roles WHERE uid=1001').fetchone()[0], self.baseline)
            with self.assertRaises(sqlite3.IntegrityError):
                db.execute('UPDATE lab_inventory SET permanent=2 WHERE instance_id=7')

    def test_permanent_policy_changes_only_weapon_presentation_and_keeps_equipment(self):
        self.store.grant_weapons(1001, self.baseline, CATALOG)
        self.store.equip_weapon(1001, 8)
        before = self.inventory_rows()
        self.assertEqual(self.store.set_weapons_permanent(1001), 3)
        after = self.inventory_rows()
        for old, new in zip(before, after):
            self.assertEqual(old[:4], new[:4])
            if old[3] != 25:
                self.assertEqual(old, new, 'A clothing item was made permanent')
                continue
            self.assertEqual(new[5], 1)
            expected = bytearray(old[4])
            struct.pack_into('<I', expected, 13, PERMANENT_DISPLAY_MINUTES)
            struct.pack_into('<I', expected, 19, 1)
            struct.pack_into('<H', expected, 23, 0)
            self.assertEqual(new[4], bytes(expected))
        self.assertEqual([struct.unpack_from('<I', item)[0]
                          for item in records(self.store.get_items(1001, self.baseline))
                          if struct.unpack_from('<H', item, 17)[0] == 8], [8])

    def test_permanent_reconnect_grant_equip_and_unequip_never_restore_consumable_count(self):
        self.store.grant_weapons(1001, self.baseline, CATALOG)
        self.store.set_weapons_permanent(1001)
        expected_rows = self.inventory_rows()
        expected_items = self.store.get_items(1001, self.baseline)
        self.assertEqual(self.store.set_weapons_permanent(1001), 3)
        self.assertEqual(self.inventory_rows(), expected_rows)
        reopened = InventoryStore(self.path, create_schema=False)
        self.assertEqual(reopened.get_items(1001, self.baseline), expected_items)
        reopened.grant_weapons(1001, self.baseline, CATALOG)
        self.assertEqual(reopened.get_items(1001, self.baseline), expected_items)
        changed = reopened.equip_weapon(1001, 8) + reopened.unequip_weapon(1001, 8)
        for item in changed:
    ...
    def test_permanent_output_uses_flag_instead_of_stale_remaining_minutes(self):
        self.store.grant_weapons(1001, self.baseline, CATALOG)
        self.store.set_weapons_permanent(1001)
        with closing(sqlite3.connect(self.path)) as db, db:
            payload = bytearray(db.execute('SELECT payload68 FROM lab_inventory WHERE instance_id=8').fetchone()[0])
            struct.pack_into('<I', payload, 13, 1)
            struct.pack_into('<H', payload, 23, 7)
            db.execute('UPDATE lab_inventory SET payload68=? WHERE instance_id=8', (bytes(payload),))
        stored = self.inventory_rows()
        for _ in range(3):
            item = records(self.store.get_items(1001, self.baseline))[7]
            self.assertEqual(struct.unpack_from('<I', item, 13)[0], PERMANENT_DISPLAY_MINUTES)
            self.assertEqual(struct.unpack_from('<H', item, 23)[0], 0)
        self.assertEqual(self.inventory_rows(), stored, 'Reading inventory unexpectedly rewrote stored items')

    def test_permanent_marking_is_uid_scoped_and_transactional(self):
        self.store.grant_weapons(1001, self.baseline, CATALOG)
        before = self.inventory_rows()
        self.assertEqual(self.store.set_weapons_permanent(1002), 0)
        self.assertEqual(self.inventory_rows(), before)
        with closing(sqlite3.connect(self.path)) as db, db:
            db.execute('''CREATE TRIGGER fail_permanent_mark BEFORE UPDATE ON lab_inventory
                          WHEN NEW.instance_id=8
                          BEGIN SELECT RAISE(ABORT, 'simulated failed permanent mark'); END''')
        with self.assertRaises(sqlite3.IntegrityError):
            self.store.set_weapons_permanent(1001)
        self.assertEqual(self.inventory_rows(), before)
