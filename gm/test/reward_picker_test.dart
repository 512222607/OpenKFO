...
void main() {
  testWidgets(
    'tutorial draft selects client names and currencies without IDs',
    (t) async {
      ...
      Future<dynamic> api(Map<String, dynamic> r) async {
        switch (r['operation']) {
          ...
          case 'shop_images':
            imageRequests++;
            expect(r['keys'], ['25:253001']);
            return {
              '25:253001': 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQIHWP4z8DwHwAFgAI/ScLbtAAAAABJRU5ErkJggg==',
            };
          case 'definitions_get':
            return [
              {'key': 321, 'record': base64Encode(record), 'days': 7},
            ];
          default:
            throw StateError('Unexpected mutation: $r');
        }
      }
      ...
      await t.tap(find.text('选择武器 / 道具 / 称号'));
      await t.pumpAndSettle();
      await t.enterText(find.byType(TextField).last, '凌云');
      await t.pumpAndSettle();
      await t.tap(find.text('凌云神腕'));
      await t.pumpAndSettle();
      expect(find.textContaining('凌云神腕'), findsOneWidget);
      expect(find.text('物品定义编号'), findsNothing);
      expect(imageRequests, 1);
      expect(find.byType(Image), findsOneWidget);
      await t.tap(find.text('应用到草稿'));
      await t.pumpAndSettle();
      expect(result, [
        {
          'items': [321],
